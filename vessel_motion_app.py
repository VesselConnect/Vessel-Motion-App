...
def compute_response(omega, rao_point, hs, tp, gamma, duration_hours, df=None):
    ...
    if df is not None:
        ...
        result['RMS heave (m)'] = np.sqrt(trapz_compat(S_heave, omega_df))
        result['RMS roll (deg)'] = np.sqrt(trapz_compat(S_roll, omega_df)) * 180.0 / np.pi
        result['RMS pitch (deg)'] = np.sqrt(trapz_compat(S_pitch, omega_df)) * 180.0 / np.pi

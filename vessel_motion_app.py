...
def parse_sheet(raw, cols):
    ...
    selected = raw.iloc[data_start_row:, [
        cols['period_col'],
        cols['heave_amp_col'],
        cols['heave_phase_col'],
        cols['roll_amp_col'],
        cols['roll_phase_col'],
        cols['pitch_amp_col'],
        cols['pitch_phase_col'],
    ]].copy()
    selected.columns = [
        'T',
        'heave_amp',
...
def compute_response(omega, rao_point, hs, tp, gamma, duration_hours, df=None):
    ...
    if df is not None:
        # FIX: Use the same omega array passed in - avoids dual grid mismatch
        heave_amp = df['heave_amp'].to_numpy(dtype=float)
        roll_amp_rad = df['roll_amp_deg_per_m'].to_numpy(dtype=float) * np.pi / 180.0
        pitch_amp_rad = df['pitch_amp_deg_per_m'].to_numpy(dtype=float) * np.pi / 180.0
        # Sort by omega to match the input omega ordering
        T = df['T'].to_numpy(dtype=float)
        order = np.argsort(2.0 * np.pi / T)
        heave_amp = heave_amp[order]
        roll_amp_rad = roll_amp_rad[order]
        pitch_amp_rad = pitch_amp_rad[order]
        S_heave = (heave_amp ** 2) * S_eta
        S_roll = (roll_amp_rad ** 2) * S_eta
        S_pitch = (pitch_amp_rad ** 2) * S_eta
        m0_heave = trapz_compat(S_heave, omega)
        m0_roll = trapz_compat(S_roll, omega)
        m2_roll = trapz_compat((omega ** 2) * S_roll, omega)
        m0_pitch = trapz_compat(S_pitch, omega)
        m2_pitch = trapz_compat((omega ** 2) * S_pitch, omega)
